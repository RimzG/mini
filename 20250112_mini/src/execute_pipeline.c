...
/*   Updated: 2024/12/30 23:05:48 by sel-huss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	execute_pipeline_loop(t_pipeline_data *data)
{
	int			pipefd[2];
	int			prev_pipe;
	int			next_pipe;
	...
	while (data->cmd_start < data->cmd_end)
	{
		...
		*(data->pid) = fork();
		...
		if (*(data->pid) == 0)
		{
			...
			handle_child_process(data->args, data->env, pipefd, pipe_data);
		}
...
int	execute_pipeline(char **args, char **env, int cmd_start, int cmd_end)
{
	pid_t			pid;
